...
public class SrsSocket extends Thread {
	...
	private SrsClient mSrsClient = null;
	...
	public SrsSocket(String ip, int port, SrsClient srsClient) {
		SrsSocket.ip = ip;
		SrsSocket.port = port;
		mSrsClient = srsClient;
		onWork = true;
	}
	...
	private String parseProtocol(byte[] data) {
		SrsStreamReader srsReader = new SrsStreamReader(data);
		SrsPackage srsPackage = new SrsPackage();
		srsPackage.decode(srsReader);
		return "ok";
	}
		...
		try {
			while (selector.select() > 0) {
				Set<SelectionKey> keys = selector
						.selectedKeys();
				Iterator<SelectionKey> iterator = keys
						.iterator();
	...
	private final byte[] readBuf(SelectionKey selectionKey)
			throws IOException {
